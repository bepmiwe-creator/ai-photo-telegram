...
const API_BASE = ''; // тот же origin или задайте полный URL бэкенда
let userBalance = 85;
let uploadedImages = [];
...
// Инициализация: загрузка баланса из БД при старте
...
async function fetchBalance() {
    try {
        ...
        if (res.ok) {
            ...
            userBalance = data.balance != null ? data.balance : userBalance;
            updateBalance();
            return userBalance;
        }
    } catch (e) {
...
// ========== ЭКРАН ЗАГРУЗКИ (SPINNER) ==========
// Пока идёт запрос на /generate — показывается индикатор ожидания (spinner) в overlay.
function showLoadingScreen(type, data) {
    ...
    // Для типа 'photo' — реальный запрос к API; для остальных — заглушка по таймеру
    if (type === 'photo' && data.images && data.images.length > 0) {
        doGenerationRequest(type, data).then(function(result) {
            hideLoadingScreen();
            showGenerationResult(type, Object.assign({}, data, { image_url: result.image_url, balance_after: result.balance_after }));
            addToHistoryGenerated(type, Object.assign({}, data, { image_url: result.image_url, balance_after: result.balance_after }));
            showNotification('Результат сохранён в раздел «История».');
        }).catch(function(err) {
            hideLoadingScreen();
...
function showGenerationResult(type, data) {
    ...
    if (image) {
        if (data.image_url) {
            ...
            image.src = 'https://via.placeholder.com/400x400/' + colors[Math.floor(Math.random() * colors.length)] + '/1E3A8A?text=Результат+' + randomNum;
        }
        image.alt = 'Сгенерированное изображение';
    }
    
    if (data.balance_after != null) {
        userBalance = data.balance_after;
        updateBalance();
    }
...
function checkGenerateButton() {
    ...
    const hasPhotos = uploadedImages.length >= 1 && uploadedImages.length <= 5;
    const price = calculatePrice();
    const enoughBalance = userBalance >= price;
    ...
    if (currentCategory === 'prompt') {
        isEnabled = (hasPrompt || hasPhotos) && enoughBalance;
        text = !enoughBalance ? `Недостаточно звёзд (нужно ${price})` : (hasPrompt ? `Сгенерировать за ${price} звёзд` : 'Введите промпт');
        hint = !enoughBalance ? `Нужно ${price} звёзд, у вас ${userBalance}` :
               hasPrompt ? 'Готово к генерации!' :
               hasPhotos ? 'Готово к генерации по фото!' :
               'Заполните поле "Опишите изображение" для генерации';
    } else {
        isEnabled = hasPhotos && enoughBalance;
        text = !enoughBalance ? `Недостаточно звёзд (нужно ${price})` : (hasPhotos ? `Сгенерировать за ${price} звёзд` : 'Загрузите фото');
        hint = !enoughBalance ? `Нужно ${price} звёзд, у вас ${userBalance}` : (hasPhotos ? 'Готово к генерации!' : 'Загрузите от 1 до 5 фото');
    }
    
    generateBtn.disabled = !isEnabled;
    btnText.textContent = text;
    hintText.textContent = hint;
    hintText.style.color = isEnabled ? '#4CAF50' : (enoughBalance ? '#ff9800' : '#f44336');
...
function setupRealUpload() {
    ...
    fileInput.setAttribute('aria-label', 'Выберите фото для загрузки');
    ...
    var uploadBtn = document.getElementById('upload-add-btn');
    if (uploadBtn) {
...
/**
 * handleUpload: проверяет количество фото (1–5) и обновляет баланс из БД.
 * Блокирует кнопку генерации, если звёзд недостаточно.
 */
function handleUpload(files) {
    ...
    if (files.length > remaining) {
        showNotification(`Можно загрузить от ${minFiles} до ${maxFiles} фото. Сейчас: ${uploadedImages.length}, осталось мест: ${remaining}`);
        return;
    }
...
// ========== ГЕНЕРАЦИЯ ФОТО ==========
/**
 * При клике на кнопку "Генерация":
 * - Собирает 1–5 загруженных фото пользователя
 * - Проверяет выбранный тариф (Nano или Pro)
 * - Показывает индикатор ожидания (spinner)
 * - Отправляет запрос на POST /generate
 */
function startGeneration() {
    var price = calculatePrice();
    if (price > userBalance) {
        showNotification('Недостаточно звёзд! Нужно: ' + price + ', у вас: ' + userBalance);
        return;
    }
    var photos = collectUserPhotosForGeneration();
    if (photos.length === 0 && currentCategory !== 'prompt') {
        showNotification('Загрузите от 1 до 5 фото для генерации');
        return;
    }
    var tariff = getSelectedTariff();
    var exampleId = getSelectedExampleId();
...
/** Собирает от 1 до 5 загруженных пользователем фото (массив с .file и .preview). */
function collectUserPhotosForGeneration() {
    if (!uploadedImages || uploadedImages.length === 0) return [];
    return uploadedImages.slice(0, 5);
}

/** Возвращает выбранный тариф: "nano" или "pro". */
function getSelectedTariff() {
    return selectedModel === 'pro' ? 'pro' : 'nano';
}

/** Идентификатор выбранного примера (категория или стиль) для эндпоинта. */
...
/**
 * Отправка на POST /generate: первое из 1–5 фото, example_id, тариф (nano/pro).
 * Спиннер уже показывается в showLoadingScreen до вызова этой функции.
 */
function doGenerationRequest(type, data) {
    var photos = (data.images || []).slice(0, 5);
    if (photos.length === 0) return Promise.reject(new Error('Нет фото для отправки'));
    var formData = new FormData();
    formData.append('photo', photos[0].file);
    formData.append('example_id', data.category || data.style || 'default');
    formData.append('generation_type', data.model || 'nano');
    formData.append('aspect_ratio', data.format || '5:4');
    return fetch(API_BASE + '/generate', {
        ...
        if (!res.ok) {
            return res.json().then(function(j) { throw new Error(j.detail || res.statusText); }).catch(function() {
                throw new Error(res.statusText || 'Ошибка ' + res.status);
            });
        }
        return res.json();
    });
...
function calculatePrice() {
    var price = selectedModel === 'nano' ? 7 : 35;
...
function addToHistoryGenerated(type, data) {
    if (data.balance_after != null) {
        ...
        userBalance -= data.price || 0;
    }
    updateBalance();
    
    var src = data.image_url || ('https://via.placeholder.com/300x300/' + ['E0F2FE', 'F8E1E7', 'FAF3E0'][Math.floor(Math.random() * 3)] + '/1E3A8A?text=Новое+' + type);
    var newPhoto = {
        id: Date.now(),
        src: src,
        title: type === 'photosession' ? ('Фотосессия: ' + (data.title || '')) : 
               type === 'create-own' ? 'Свой стиль' : 
               ((categories.find(function(c) { return c.id === data.category; }) || {}).title || 'Фото') + (data.style ? ' - ' + data.style : ''),
...
/**
 * Загружает историю с бэкенда (GET /api/history) и отображает в гриде «История».
 * После каждой успешной генерации ссылка сохраняется в БД и попадает сюда.
 */
function loadHistory() {
    var container = document.getElementById('history-photos-container');
    ...
    fetch(API_BASE + '/api/history', {
        ...
        .then(function(data) {
            ...
            if (list.length === 0) {
                container.innerHTML =
                    ...
                    '<p class="history-empty-title">У вас пока нет созданных шедевров</p>' +
                    '<p class="history-empty-desc">Создайте первое фото в разделе «Фото» — и оно появится здесь.</p>' +
                    '<button type="button" class="history-empty-btn" id="history-start-create-btn">Начать создавать</button>' +
                    '</div>';
                var startBtn = document.getElementById('history-start-create-btn');
                if (startBtn) {
                    startBtn.addEventListener('click', function() { switchScreen('main'); });
                }
                return;
            }
            ...
            list.forEach(function(item) {
                ...
                var dateStr = item.created_at ? new Date(item.created_at).toLocaleDateString('ru-RU') : '';
                card.innerHTML = '<div class="history-photo-card-inner">' +
                    '<img src="' + (item.image_url || '') + '" alt="' + (item.title || 'Фото') + '" loading="lazy">' +
                    '<div class="history-photo-card-info">' +
                    '<span class="history-photo-card-title">' + (item.title || 'Фото') + '</span>' +
                    '<span class="history-photo-card-date">' + dateStr + '</span>' +
                    (item.price_stars ? '<span class="history-photo-card-stars">' + item.price_stars + ' ⭐</span>' : '') +
                    '</div>' +
                    '<button type="button" class="history-photo-card-download" onclick="downloadFromHistory(\'' + (item.image_url || '') + '\')">Скачать</button>' +
                    '</div>';
        ...
        .catch(function() {
            ...
            container.innerHTML =
                '<div class="history-empty-state">' +
                '<p class="history-empty-title">Не удалось загрузить историю</p>' +
                '<p class="history-empty-desc">Проверьте подключение к интернету.</p>' +
                '<button type="button" class="history-empty-btn" id="history-retry-btn">Повторить</button>' +
                '</div>';
...
function downloadFromHistory(imageUrl) {
    if (imageUrl) {
        var a = document.createElement('a');
        a.href = imageUrl;
        a.download = 'nano-banana-photo.jpg';
        a.target = '_blank';
        a.rel = 'noopener';
        a.click();
    }
    showNotification('Ссылка на фото открыта. Сохраните изображение.');
}
